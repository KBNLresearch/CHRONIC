...
os.environ['THEANO_FLAGS']='cuda.root=/hpc/sw/cuda/7.5.18,floatX=float32,device=gpu0'
...
print("Import done, network setup")
...
outfile = open("neural_photos.pickle", "rb")
NEph = np.load(outfile)
outfile.close()

outfile = open("neural_drawings.pickle", "rb")
NEdr = np.load(outfile)
outfile.close()

nPhotos = len(glob.glob("./TestImages/photos/*.jpg"))
nDraws = len(glob.glob("./TestImages/draws/*.jpg"))
